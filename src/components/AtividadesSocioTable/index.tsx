import AtividadesSocioTableRow from '../AtividadesSocioTableRow';
import { gql, useQuery } from '@apollo/client';
import { Table, TableProps, Tbody, Th, Thead, Tr } from '@chakra-ui/react';

// eslint-disable-next-line @typescript-eslint/no-empty-interface
interface AtividadesSocioTableProps extends TableProps {}

export type ProgramacaoData = {
  id: string;
  estado: string;
  descricao: string;
  modalidade: {
    nome: string;
    categoria: string;
  };
  dataHora: string;
  local: string;
  competidoresMinimo: number;
  competidoresConfirmados: {
    edges: {
      node: {
        socio: {
          matricula: string;
        };
      };
    }[];
  };
};

const QUERY_PROGRAMACAO_ESPORTE = gql`
  {
    allProgramacao(modalidade_Categoria: "Esporte") {
      edges {
        node {
          id
          estado
          descricao
          modalidade {
            nome
            categoria
          }
          dataHora
          competidoresMinimo
          competidoresConfirmados {
            edges {
              node {
                socio {
                  matricula
                }
              }
            }
          }
          local
        }
      }
    }
  }
`;
const QUERY_PROGRAMACAO_BATERIA = gql`
  {
    allProgramacao(modalidade_Categoria: "Bateria") {
      edges {
        node {
          id
          estado
          descricao
          modalidade {
            nome
            categoria
          }
          dataHora
          competidoresMinimo
          competidoresConfirmados {
            edges {
              node {
                socio {
                  matricula
                }
              }
            }
          }
          local
        }
      }
    }
  }
`;

function AtividadesSocioTable({ ...rest }: AtividadesSocioTableProps) {
  const programacao_esporte = useQuery(QUERY_PROGRAMACAO_ESPORTE);
  const programacao_bateria = useQuery(QUERY_PROGRAMACAO_BATERIA);

  function compare_datetime(a: any, b: any) {
    if (a.node.dataHora < b.node.dataHora) {
      return -1;
    }
    if (a.node.dataHora > b.node.dataHora) {
      return 1;
    }
    return 0;
  }

  // Variable that unifies the data from both queries
  let programacao = [];
  if (programacao_esporte.data) {
    programacao = programacao_esporte.data.allProgramacao.edges;
  }
  if (programacao_bateria.data) {
    programacao = programacao.concat(
      programacao_bateria.data.allProgramacao.edges,
    );
  }

  return (
    <Table colorScheme="gray" {...rest}>
      <Thead>
        <Tr>
          <Th></Th>
          <Th>Modalidade</Th>
          <Th>Categoria</Th>
          <Th>Data/hora</Th>
          <Th>Local</Th>
          <Th>Descrição</Th>
        </Tr>
      </Thead>
      <Tbody>
        {programacao
          .sort(compare_datetime)
          .map(({ node }: { node: ProgramacaoData }) => {
            return <AtividadesSocioTableRow key={node.id} node={node} />;
          })}
      </Tbody>
    </Table>
  );
}

export default AtividadesSocioTable;
